...
%%% Using H=h omega(cos(fi)X+sin(fi)Z)/2 -Amplitude Damping
...
%---Files for data---%
FileName1=sprintf('Fmean_Tau%1.0f_r%1.0f_nu%1.0f_NR%2.0f_AD.txt',Tau,r*10,nu,NR);
fID1=fopen(FileName1,'wt');

FileName2=sprintf('wmean_Tau%1.0f_r%1.0f_nu%1.0f_NR%2.0f_AD.txt',Tau,r*10,nu,NR);
fID2=fopen(FileName2,'wt');

FileName3=sprintf('Fvsk_g_Tau%1.0f_r%1.0f_nu%1.0f_NR%2.0f_AD.txt',Tau,r*10,nu,NR);
fID3=fopen(FileName3,'wt');

FileName4=sprintf('Fvsk_e_Tau%1.0f_r%1.0f_nu%1.0f_NR%2.0f_AD.txt',Tau,r*10,nu,NR);
fID4=fopen(FileName4,'wt');

FileName5=sprintf('DecTime_Tau%1.0f_r%1.0f_nu%1.0f_NR%2.0f_AD.txt',Tau,r*10,nu,NR);
...
for idt=1:NDT    
    ...
    %---Krauss operators E for Amplitude Damping---%
    E0 = Au_g*Au_g'+exp(-gamma*Tau)*Au_e*Au_e' ;
    E1 = sqrt(1-exp(-2*gamma*Tau))*Au_g*Au_e';
    ...
    for ir=1:NR
        ...
        F_kg=zeros(1,N); % Fidelity the 1st eigenvector of H.
        F_ke=zeros(1,N); % Fidelity the 2d eigenvector of H.
        ...
        F_g(:,idt)=F_gmean; % Mean fidelity for first eigenvalue of OE for different dt. 
        F_e(:,idt)=F_emean; % Mean fidelity for second eigenvalue of OE for different dt.
        F_DT(:,idt)=F_mean;   % Mean fidelity global for different Gammas.
        w_DT(:,idt)=w_mean;   % R/T rate for different ddt.

        lim_F(idt) = mean(F_mean(N-10:N)); 
        lim_W(idt) = mean(w_mean(N-10:N));
        
        fprintf(fID1,' %14.12f',F_DT(:,idt)); % Extra check: Save the fidelity. Each file is a different DT
        fprintf( fID1,'%6s\n','');
        
        fprintf(fID2,' %14.12f',w_DT(:,idt)); % Extra check: Save the fidelity. Each file is a different lDT
        fprintf( fID2,'%6s\n','');
      
        fprintf(fID3,' %14.12f',F_g(:,idt)); % Extra check: Save the fidelity. Each file is a different DT
        fprintf(fID3,'%6s\n','');  
    
        fprintf(fID4,' %14.12f',F_e(:,idt)); % Extra check: Save the fidelity. Each file is a different DT
        fprintf(fID4,'%6s\n','');
...
plot(k_it, F_DT); hold on
plot(k_it,w_DT); hold on
...
subplot(2,1,2)
%plot(k_it,w_DT); 
plot(k_it, F_g); hold on
plot(k_it, F_e); hold on
lgd=legend( sprintfc('%g', DecTime_NDT) );
title(lgd,'\lambda')
xlabel('$k$', 'interpreter', 'latex'); 
